import type { ExcelContextSnapshot, RiskAssessment } from "./context-snapshot";
import type {
  AgentPlan,
...
export class BasicPlanValidator implements PlanValidator {
  validate(
    ...
    if (context.safety.flags.readOnlyMode) {
      issues.push(
        "Workbook is in read-only mode; write operations are not allowed."
      );
    }
    ...
    const isValid = issues.length === 0 && baseRisk.level !== "high";
